...
def getRecentPostings(dbName, numDaysAge=4):
    pastDate = (datetime.datetime.now() - datetime.timedelta(
                    days=numDaysAge)).strftime('%Y-%m-%d')
    res = getResultDictForSQL(("select Id as id, URL as url, Title as title, "
                               "Company as company, Locale as locale, "
                               "SearchTerms as searchTerms, ElementHtml as "
                               "elem, postedDate, insertedDate from "
                               "JobPostings where postedDate > '{0}' and "
                               "insertedDate > '{0}'").format(pastDate),
                              databaseName=dbName)
    print('Numbr of res returned: {}'.format(len(res)))
    # print(res[0])
    for item in res:
        item['elem'] = BeautifulSoup(item['elem'], 'html.parser')
...
def generateHtmlPage(dirPath, netloc, searchTermsList, postingsList):
    ...
    with open(filenamePath, 'w', encoding='utf-8') as myFile:
        myFile.write(('<html><head><style>\n.job-tools-description { '
                      'padding: 5px; } \n.job-tools-apply { padding: 5px; }'
                      '\n.recent-date { font-weight: bold; background-color: '
                      'greenyellow; } \n.desired-location { font-weight: bold; '
                      'background-color: greenyellow; \n}</style></head><body><p><h2>'))
        myFile.write('Search Site:</h2> {}</p>\n'.format(netloc))
        myFile.write('<p><h2>Search Terms:</h2>\n<ul>\n')
        for st in searchTermsList:
            myFile.write('<li>{}</li>\n'.format(st))
        myFile.write('</ul></p>\n\n<hr/>\n<p>')
        myFile.write('Found {} Postings.</p>\n\n'.format(len(postingsList)))
        myFile.write('<hr/>\n\n<p>Locations:</p><ul>')
        locList = [p['locale'] for p in postingsList]
        locations = set(locList)
        for loc in sorted(locations):
            myFile.write('<li>{} : {}</li>'.format(loc, locList.count(loc)))
        myFile.write('</ul><hr/>\n')
        ...
        for posting in sortedPostings:
            try:
                for anch in posting['elem'].findAll('a'):
                    ...
                    anch['target'] = '_blank'
                locale = posting['locale'] if posting.get('locale') else ''
                localeClass = 'desired-location' if locale.lower() in ('ottawa, on', ) else ''
                dataClass = 'recent-date' if False else ''
                myFile.write(('<ul><li>ID: {0}</li><li>Title: {1}</li><li>URL'
                              ': <a href="{2}" target="_blank">{2}</a></li><li>Company: {3}'
...
def getJobPostingsFromSite(jobSiteDetailsInfo, searchTerm, knownPostIds=[],
    ...
    postingsList, numPostingsOnPage, initTotalNumPostings = parseHtmlPage2(
            page.text, jobSiteDetailsInfo, searchTerm, knownPostIds, verbose)
    ...
    while checkForMorePostings(numPostingsOnPage, expectedPostingsPerPage,
        ...
        postingsList, numPostingsOnPage, totalNumberJobsFound = parseHtmlPage2(
                page.text, jobSiteDetailsInfo, searchTerm,
                knownPostIds, verbose)
        fullPostingsList.update(postingsList)
        knownPostIds.extend(list(postingsList.keys()))
...
def scrape_new_job_postings():
    ...
    for jobSiteDetailsInfo in siteList.values():
        fullPostingsList.update(
            getJobPostingsFromSiteForMultipleSearchTerms(
                jobSiteDetailsInfo,
                searchTermsList,
                knownPostIds))
    ...
    for post_id in fullPostingsList:
        ...
        try:
            ...
            print('Exception %s occurred while trying to insert posting %s %s %s.' %
                  (e, posting['id'], posting['title'], posting.get('company')))
...
def main(args):
    ...
    parser.add_argument('--verbose', '-v', action='store_const',
                        const=True, default=None,
                        help=('Display extra information during the running '
                              'of this script.'))
    ...
    if args.generateReport:
        fullPostingsList = getRecentPostings(dbName, 2)
    else:
        knownPostIds = []
